...
def lab_sessions():
    now = datetime.now()
    available_sessions = (
        CaThucHanhModel.query.filter(
            CaThucHanhModel.dang_hoat_dong.is_(True), CaThucHanhModel.ngay >= now.date(), CaThucHanhModel.gio_bat_dau > now
        )
        .order_by(CaThucHanhModel.ngay, CaThucHanhModel.gio_bat_dau)
        .all()
    )
    registered_sessions = (
        CaThucHanhModel.query.join(DangKyCaModel)
        .filter(DangKyCaModel.nguoi_dung_ma == current_user.id)
        .order_by(CaThucHanhModel.ngay, CaThucHanhModel.gio_bat_dau)
        .all()
    )
...
def verify_lab_session(session_id):
    ...
    if vao_ca:
        return redirect(url_for("lab.lab_session_active", entry_id=vao_ca.ma))
...
def lab_session_active(entry_id):
    vao_ca = VaoCaModel.query.filter_by(ma=entry_id, nguoi_dung_ma=current_user.id).first_or_404()
...
def my_lab_sessions():
    ...
    lich_su_vao_ca = VaoCaModel.query.filter_by(nguoi_dung_ma=current_user.id).order_by(VaoCaModel.thoi_gian_vao.desc()).all()
    return render_template("lab/my_sessions.html", sessions=registered_sessions, lab_history=lich_su_vao_ca)
...
def admin_lab_sessions():
    ca_thuc_hanh_list = CaThucHanhModel.query.order_by(CaThucHanhModel.ngay.desc(), CaThucHanhModel.gio_bat_dau).all()
    return render_template("admin/admin_lab_sessions.html", sessions=ca_thuc_hanh_list)
...
def lab_session_attendees(session_id):
    ca_thuc_hanh = CaThucHanhModel.query.get_or_404(session_id)
    dang_ky_list = DangKyCaModel.query.filter_by(ca_thuc_hanh_ma=session_id).join(NguoiDungModel).order_by(NguoiDungModel.ten_nguoi_dung).all()
    vao_ca_list = VaoCaModel.query.filter_by(ca_thuc_hanh_ma=session_id).join(NguoiDungModel).order_by(VaoCaModel.thoi_gian_vao).all()
    return render_template(
        "admin/admin_session_attendees.html", session=ca_thuc_hanh, registrations=dang_ky_list, entries=vao_ca_list
    )
...
def schedule_lab_sessions():
    valid_sessions = (
        db.session.query(CaThucHanhModel, func.count(DangKyCaModel.ma).label("student_count"))
        .join(DangKyCaModel)
        .group_by(CaThucHanhModel.ma)
        .having(func.count(DangKyCaModel.ma) >= 5)
        .order_by(CaThucHanhModel.ngay, CaThucHanhModel.gio_bat_dau)
        .all()
    )
    pending_sessions = (
    ...
    return render_template(
        "admin/admin_schedule_sessions.html", valid_sessions=valid_sessions, pending_sessions=pending_sessions
    )
